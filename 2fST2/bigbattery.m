...
% MAHAN: Do we want to save individual neuron's excitatory postsyn. activations or membrane potentials (1:5 based on what to save)?
global SaveNeurAct
global guesses NUMITEMS %MAHAN: Make model guess more often to replicate Vul
...
if(don0t)
    ...
    if SaveNeurAct == 1
        %MAHAN: add per neuron variable
        ExPostsynFull_basic = zeros(numTcombi,NUMSTREAMS,runlength,NUMLAYERS, 100); % 100 neurons because numrespbinders = numneurons*numWM(4) & numneurons = numtargs(10) + numdistractors(15) [even though tokens other than 1st are not used]
    elseif SaveNeurAct == 2
    ...
    %MAHAN: initialise allguesses variable to make model guess more often (4 Vul)
    allguesses = zeros(numTcombi,NUMSTREAMS, NUMITEMS);
        ...
        for m=1:numrespvals
            for k=1:numrespvals
                ...
                for j=T2start:T2stop
                    ...
                    %MAHAN: Store guesses of this trial (across all 4 "lags") to allguesses variable
                    allguesses(trialcounter,:,:) = guesses;
                    % => third dimension is "guess" dimension - NaN if
                    % nothing was bound, otherwise:
                    %   1st index - response
                    %   2nd index - first guess (or just 2nd type if only 2 types bound)
                    %   3rd index - second guess (or just 3rd type if only 3 types bound)
                    %   and so on
                    
                    % ALL GUESSES HAS TYPE NUMBER IN IT EVEN IF TRIAL WAS A
                    % MISS - JUST MAKE THOSE NANs (as in initialisation of
                    % guesses) & implemnent sanitycheck when using
                    % keepepochs to throw out those indices. should all be
                    % nans!
...
% MAHAN: save allguesses variable to replicate Vuls stuff
save('Guesses', 'allguesses')
