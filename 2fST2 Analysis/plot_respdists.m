function plot_respdists(name, varpath, plotpath, saveplots, TwoRespFeatures)
%% Get numerical response distributions
switch TwoRespFeatures
    case 0
        this_varpath = [varpath 'Srivas/'];
    case 1
        this_varpath = [varpath 'Alon/'];
end
[marker, ~, ~] = extract_responses(name, this_varpath, TwoRespFeatures);
...
switch TwoRespFeatures
    case 0
        xvals = -2:1:2;
        suptitle = ['Srivas'' Model - ' name];
        savename = ['Srivas'' - ' name ' - Response Distributions.jpg'];
    case 1
        xvals = 0:1;
        suptitle = ['My Model - ' name];
...
ax.FontSize = 25;
if TwoRespFeatures
    ax.XTickLabels = {'Correct', 'Intrusion'};
end
ax.FontSize = 35;
%% Do the labels
hold on
[~, h1] = suplabel('Proportion', 'y');
[~, h3] = suplabel(suptitle, 't');
