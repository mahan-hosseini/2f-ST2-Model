...
global BIAS LEAK DT_VM EE EI EL GAMMA THETA NUMLAYERS NUMWM TDUK TDUWEIGHT STARTVAL NUMSTREAMS NUMINPUTS THETA2 THETA3 % MAHAN: note TDUK (by me) is still hidden in this row among the other original global vars
...
global guesses                       %MAHAN: For making model guess more often if multbinds
if SaveNeurAct == 1
...
global GammaDist %MAHAN: Add boost to blaster to stimulate human N2pc-variability
% global  BlasterBoost %MAHAN: old
...
%         %Allow special adjustment for Task Demand Unit in layer 4
    ...
    %MAHAN: BEFORE the step-loop, implement the variability to keydelay to
    %be used below to define keyshift (HAS TO BE BEFORE step-loop!)
    thistrial_keyd_var = GammaDist(randi(numel(GammaDist))); % sample randomly from GammaDist
    thistrial_keyd_var = round(thistrial_keyd_var); % make sure it's an integer!
    thistrial_keydelay = keydelay + thistrial_keyd_var;
    ...
    for step = 1:runlength
        ...
        %MAHAN: IMPLEMENTING variability to keydelay via the use of
        %   thistrial_keydelay instead of keydelay for Retina shift stuff
        %   (original implementation of fixed keydelay)
        %   ==> Has to be done here and below when the TDUs come into play!
        %   ==> All following steps (except these two) do not depend on
        %       keydelay!
        
        %Retina is smaller than number of timesteps 
        keyshift = step-thistrial_keydelay;
        ...
        for layer = 2:NUMLAYERS %vertical loop
            ...
            for neuron = 1:NUMNEURONSBYLAYER(layer) %horizontal loop
                ...
                if layer == 4 && TDUK(neuron) > 0 
                    if thistrial_keydelay > 0 && step <= thistrial_keydelay
                        bias(layer,neuron) = 0;
                    else
...
%
%                                       OLD 
%                               BLAST IN BOOST IS HERE
%
%
%
%
                %MAHAN: We add gamma-distributed variance to blaster here
                % ==> Let's also make the scale of our gamma function be depending on fixed keydelay config (tau_k)
                % ==> Implementing it in BlasterIn layer to make sure that
                %   BlasterInShutoff is affected by this exactly as
                %   BlasterOut is! (should be problematic if we implement
                %   this in BlasterOut, for example)
                % ==> We also have to make sure that enhancement is only
                % given during time-period of blaster-activation!
                %   -- For this, we use sigout (type 1) for both directions 
                %       of BlastIn to BlastInOff connection, because:
                %   if In=>Off means blaster has been excited by TFL (key feature present), so we start boosting
                %   if Off=>In means we want to shutoff act, so we stop boosting
                % ==> The bigger gamma scale (below) is, the more likely
                %   it is that the bias gets a stronger boost (i.e.
                %   blaster-firing latency dist gets narrower), so we
                %   use smaller values to simulate the fact that in
                %   humans N2pc-var gets broader as keypathway
                %   processing is slowed down!
%                 if sigout(11,1) && ~sigout(12,1)  % restricting time-range of enhancement as described above here
%                     if layer == 11 && neuron == 1 % ==> Blaster-circuit only has 1 neuron always being the first
%                         this_boost = BlasterBoost(randi(numel(BlasterBoost))); % ==> randomly take a value out of BlasterBoost
%                         bias(layer,neuron) = bias(layer,neuron) * this_boost; 
%                     end
%                 end
